...
export async function GET(request: NextRequest) {
  ...
  const rateKey = `analytics:views:${ip}`;
  const rate = checkRateLimit(rateKey, { windowMs: 60_000, max: 60 });
  if (!rate.allowed) {
    return NextResponse.json(
      {
        error: "Too many requests",
        details: "Rate limit exceeded for /analytics/views",
      },
      {
        status: 429,
        headers: {
  ...
  try {
    const metrics = await getViewTimeSeries(range);
    return NextResponse.json(
      {
        success: true,
        range,
    ...
    return NextResponse.json(
      {
        error: "Failed to load view analytics",
